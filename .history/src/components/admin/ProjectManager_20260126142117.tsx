...
import { uploadImage } from '../../utils/upload'; // Nhớ đảm bảo file này tồn tại
import { Plus, Trash2, Save, X, Image as ImageIcon } from 'lucide-react';

export default function ProjectManager() {
  const [projects, setProjects] = useState<Project[]>([]);
  const [selectedProject, setSelectedProject] = useState<Project | null>(null); // Project đang chọn để edit
  ...
  // -- HELPER COMPONENTS FOR STYLE --
  const VibeLabel = ({children}: {children: React.ReactNode}) => (
    <label className="block font-vibe uppercase tracking-tighter leading-none text-xs text-gray-500 mb-1">{children}</label>
  );
  
  const VibeInput = (props: any) => (
    <input {...props} className="w-full p-2 border border-gray-200 font-vibe font-medium outline-none focus:border-black focus:bg-gray-50 transition-all text-sm" />
  );

  // Logic thêm item cục bộ (chưa lưu DB)
// Logic thêm item cục bộ
  const handleAddItem = () => {
    if (!selectedProject) return;
    
    // Tạo object mới và ép kiểu "as any" để TypeScript không báo lỗi thiếu id
    const newItem: any = {
        project_id: selectedProject.id,
        title: 'New Item',
        priority: 0,
    ...
    setSelectedProject({
        ...selectedProject,
        items: [newItem, ...selectedProject.items] // Thêm lên đầu danh sách
    });
  };
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <div className="col-span-2">
                                ...
                                <VibeInput 
                                    ...
                                    className="w-full p-3 border border-gray-200 font-vibe font-medium outline-none focus:border-black text-xl"
                                />
                            </div>
                            ...
                            <div>
                                ...
                                <select 
                                    ...
                                    className="w-full p-3 border border-gray-200 font-vibe font-medium outline-none focus:border-black bg-white uppercase"
                                >
